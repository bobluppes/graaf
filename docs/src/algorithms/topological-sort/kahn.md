...
- **return** Vector of vertices sorted in topological order. If the graph contains a cycle, returns `std::nullopt`.
...
Both report a cycle the same way, by returning `std::nullopt`, so callers can switch between the two without changing
how they handle a cyclic graph.
