...
static void bm_add_primitive_numeric_vertex(benchmark::State& state) {
  const auto number_of_vertices{static_cast<size_t>(state.range(0))};

  graaf::directed_graph<int, int> graph{};
  for (auto _ : state) {
    for (size_t i{0}; i < number_of_vertices; ++i) {
      [[maybe_unused]] const auto vertex_id{graph.add_vertex(i)};
    }
  }
}
...
static void bm_add_user_defined_vertex(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (size_t i{0}; i < number_of_vertices; ++i) {
      [[maybe_unused]] const auto vertex_id{graph.add_vertex(vertex{})};
    }
  }
}
