...
namespace graaf::algorithm {
  ...
  namespace {
    ...
    struct record_vertex_callback {
      ...
      record_vertex_callback(seen_vertices_t& seen_vertices, vertex_order_t& vertex_order) 
  ...
  TYPED_TEST(TypedGraphTraversalTest, MinimalGraphDFS) {
    ...
    // WHEN
    traverse<search_strategy::DFS>(graph, vertex_1, record_vertex_callback{seen_vertices, vertex_order});
  ...
  TYPED_TEST(TypedGraphTraversalTest, MinimalGraphBFS) {
    ...
    // WHEN
    traverse<search_strategy::BFS>(graph, vertex_1, record_vertex_callback{seen_vertices, vertex_order});
  ...
  TYPED_TEST(TypedGraphTraversalTest, MoreComplexGraphDFS) {
    ...
    // THEN
    const seen_vertices_t expected_vertices{vertex_1, vertex_2, vertex_3, vertex_4, vertex_5};
    ASSERT_EQ(seen_vertices, expected_vertices);
  ...
  TYPED_TEST(TypedGraphTraversalTest, MoreComplexGraphBFS) {
    ...
    // We do BFS, so all immediate neighbors must be traversed first before going deeper in the graph
    ASSERT_TRUE(vertex_order.at(vertex_2) < vertex_order.at(vertex_4) && vertex_order.at(vertex_3) < vertex_order.at(vertex_4));
    ASSERT_TRUE(vertex_order.at(vertex_2) < vertex_order.at(vertex_5) && vertex_order.at(vertex_3) < vertex_order.at(vertex_5));
  }
  ...
  TEST(GraphTraversalTest, MoreComplexDirectedGraphEdgeWrongDirectionDFS) {
    ...
    // THEN
    const seen_vertices_t expected_vertices{vertex_1, vertex_2, vertex_3, vertex_4};
    ASSERT_EQ(seen_vertices, expected_vertices);
  }
