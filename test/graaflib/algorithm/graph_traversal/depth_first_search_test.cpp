...
TYPED_TEST(TypedGraphTraversalTestDFS, MinimalGraphDFS)
{
    ...
    // WHEN
    depth_first_traverse(graph, vertex_1, record_edge_callback {seen_edges, edge_order});
...
TYPED_TEST(TypedGraphTraversalTestDFS, MoreComplexGraphDFSImmediateTermination)
{
    // GIVEN
    using graph_t = typename TestFixture::graph_t;
    auto const [graph, vertex_ids] {utils::scenarios::create_tree_scenario<graph_t>()};
