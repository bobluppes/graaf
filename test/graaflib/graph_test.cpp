...
TYPED_TEST(GraphTest, VertexCount)
{
    ...
    // WHEN - THEN
    int constexpr specific_id = 2;
    auto const vertex_specific_id {graph.add_vertex(30, specific_id)};
...
TYPED_TEST(GraphTest, RemoveVertex)
{
    ...
    auto const invalid_vertex_id = vertex_id_1 + vertex_id_3 + 1;
...
TYPED_TEST(GraphTest, AddEdge)
{
    ...
    graph_t graph {};
    auto const vertex_id_1 {graph.add_vertex(10)};
    auto const vertex_id_2 = graph.vertex_count() + 1;
...
TYPED_TEST(GraphTest, VertexTests)
{
    ...
    auto const nonExistingVertexId = graph.vertex_count() + 1;
    ...
    ASSERT_THROW(
        {
            try
            {
                ...
                [[maybe_unused]]
                auto const vertex {graph.get_vertex(nonExistingVertexId)};
    ...
    ASSERT_THROW(
        {
            try
            {
                ...
                [[maybe_unused]]
                auto const duplicate_id {graph.add_vertex(50, vertex_id_1)};
                FAIL() << "Expected std::invalid_argument exception, but no exception "
                          "was thrown.";
            }
            catch(std::invalid_argument const& ex)
            {
                EXPECT_EQ(ex.what(),
                          fmt::format("Vertex already exists at ID [{}]", vertex_id_1));
                throw;
            }
        },
        std::invalid_argument);
    ...
    ASSERT_THROW(
        {
            try
            {
                ...
                [[maybe_unused]]
                auto const edge {graph.get_edge(vertex_id_1, nonExistingVertexId)};
...
TYPED_TEST(GraphTest, ConstGetter)
{
    ...
    auto const test_getters_on_const_graph {
        [vertex_id_1, vertex_id_2](graph_t const& const_graph)
        {
...
class copy_recorder
{
  public:
    ...
    // Copy ctor
    copy_recorder(copy_recorder const&)
        : copied_ {true}
    {}

    // Move ctor
    copy_recorder(copy_recorder&&) noexcept
        : copied_ {false}
    {}

    [[nodiscard]]
    bool is_copy_constructed() const
    {
        return copied_;
    }
...
TEST_F(MoveTest, VertexLvalueShouldNotBeMoved)
{
    ...
    auto const id {graph_.add_vertex(moveable_)};
...
TEST_F(MoveTest, VertexRvalueShouldBeMoved)
{
    ...
    auto const id {graph_.add_vertex(std::move(moveable_))};
...
TEST_F(MoveTest, EdgeLvalueShouldNotBeMoved)
{
    ...
    auto const lhs_vertex {graph_.add_vertex(vertex_t {})};
    auto const rhs_vertex {graph_.add_vertex(vertex_t {})};
