...
TEST(GraphTest, GetTransposedGraph) {
  // GIVEN
  directed_graph<int, int> original_graph{};
  const auto vertex_id_1 = original_graph.add_vertex(1);
  const auto vertex_id_2 = original_graph.add_vertex(2);
  const auto vertex_id_3 = original_graph.add_vertex(3);
  original_graph.add_edge(vertex_id_1, vertex_id_2, 100);
  original_graph.add_edge(vertex_id_2, vertex_id_3, 200);
  original_graph.add_edge(vertex_id_3, vertex_id_1, 300);

  // WHEN
  directed_graph<int, int> transposed_graph{};
  get_transposed_graph(original_graph, transposed_graph);

  // THEN
//  std::unordered_map<vertex_id_t, vertex_id_t> vertex_id_mapping;
//  for (const auto &[original_vertex_id, transposed_vertex_id] :
//       transposed_graph.get_vertices()) {
//    vertex_id_mapping.insert({})
//        [transposed_vertex_id] = original_vertex_id;
//  }
//
//  ASSERT_TRUE(vertex_id_mapping.count(vertex_id_1));
//  ASSERT_TRUE(vertex_id_mapping.count(vertex_id_2));
//  ASSERT_TRUE(vertex_id_mapping.count(vertex_id_3));
//
//  // Verify edges using transposed graph's vertex IDs
//  ASSERT_EQ(transposed_graph.edge_count(), 3);
//  ASSERT_TRUE(transposed_graph.has_edge(vertex_id_mapping[vertex_id_2],
//                                        vertex_id_mapping[vertex_id_1]));
//  ASSERT_TRUE(transposed_graph.has_edge(vertex_id_mapping[vertex_id_3],
//                                        vertex_id_mapping[vertex_id_2]));
//  ASSERT_TRUE(transposed_graph.has_edge(vertex_id_mapping[vertex_id_1],
//                                        vertex_id_mapping[vertex_id_3]));
//
//  // Verify edge weights
//  EXPECT_EQ(transposed_graph.get_edge(vertex_id_mapping[vertex_id_2],
//                                      vertex_id_mapping[vertex_id_1]),
//            100);
//  EXPECT_EQ(transposed_graph.get_edge(vertex_id_mapping[vertex_id_3],
//                                      vertex_id_mapping[vertex_id_2]),
//            200);
//  EXPECT_EQ(transposed_graph.get_edge(vertex_id_mapping[vertex_id_1],
//                                      vertex_id_mapping[vertex_id_3]),
//            300);
//}
