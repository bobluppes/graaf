...
std::size_t vertex_degree(graaf::graph<V, E, T> const& graph, vertex_id_t vertex_id)
{
    ...
    {
        return vertex_outdegree(graph, vertex_id) + vertex_indegree(graph, vertex_id);
    }
...
std::size_t vertex_indegree(graaf::graph<V, E, T> const& graph, vertex_id_t vertex_id)
{
    ...
    if constexpr(T == graph_type::DIRECTED)
    {
        return std::ranges::count_if(
            graph.get_vertices(),
            [&graph, vertex_id](typename vertex_id_to_vertex_t::value_type const& kv_pair)
            {
                auto const& [current_vertex_id, _] {kv_pair};
