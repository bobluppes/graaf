...
template<typename V, typename E, graph_type T, typename HEURISTIC_T, typename WEIGHT_T>
                                                  ...
                                                  const HEURISTIC_T& heuristic)
{
    ...
    using a_star_queue_t     = std::priority_queue<weighted_path_item,
    ...
    while(!open_set.empty())
    {
        ...
        for(auto const& neighbor: graph.get_neighbors(current.id))
        {
            ...
            if(!vertex_info.contains(neighbor) || tentative_g_score < g_score[neighbor])
            {
