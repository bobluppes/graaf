...
namespace detail
{
    ...
    bool do_dfs(graph<V, E, T> const& graph,
                ...
                const SEARCH_TERMINATION_STRATEGY_T& search_termination_strategy)
    {
        ...
        for(auto neighbor_vertex: graph.get_neighbors(current))
        {
            if(!seen_vertices.contains(neighbor_vertex))
            {
                ...
                {
                    // Further down the call stack we have hit the search termination
                    // point. Bubble this up the call stack.
                    return false;
                }
            }
        }
    ...
    const SEARCH_TERMINATION_STRATEGY_T& search_termination_strategy)
{
    std::unordered_set<vertex_id_t> seen_vertices {};
    detail::do_dfs(
        graph, seen_vertices, start_vertex, edge_callback, search_termination_strategy);
}
