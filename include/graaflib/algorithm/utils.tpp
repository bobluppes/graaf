    ...
    directed_graph<VERTEX_T, EDGE_T> const& graph)
{
    ...
    for(auto [edge_vertices, edge_type]: graph.get_edges())
    {
        ...
        {
            auto vertex_id =
                transposed_graph.add_vertex(std::move(vertex_value_lhs), vertex_id_lhs);
        }

        if(!transposed_graph.has_vertex(vertex_id_rhs))
        {
            auto vertex_id =
                transposed_graph.add_vertex(std::move(vertex_value_rhs), vertex_id_rhs);
        }

        transposed_graph.add_edge(vertex_id_rhs, vertex_id_lhs, std::move(edge_type));
    }
