...
namespace detail
{
    ...
    void do_dfs_topological_sort(graph<V, E, graph_type::DIRECTED> const& graph,
                                 ...
                                 std::vector<vertex_id_t>& sorted_vertices)
    {
        ...
        for(auto const& next_vertex: graph.get_neighbors(start_vertex))
        {
            if(!processed_vertices.contains(next_vertex))
            {
                do_dfs_topological_sort(
                    graph, next_vertex, processed_vertices, sorted_vertices);
            }
        }
    ...
    graph<V, E, graph_type::DIRECTED> const& graph)
{
    ...
    for(auto const& [vertex_id, _]: graph.get_vertices())
    {
        ...
        {
            detail::do_dfs_topological_sort(
                graph, vertex_id, processed_vertices, sorted_vertices);
        }
    }
