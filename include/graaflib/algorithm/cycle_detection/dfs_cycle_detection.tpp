...
namespace detail
{
    ...
    bool do_dfs_directed(graph<V, E, graph_type::DIRECTED> const& graph,
                         std::unordered_map<vertex_id_t, vertex_color>& colored_vertices,
                         vertex_id_t current)
    {
        colored_vertices[current] = vertex_color::VISITED;

        for(auto const& neighbour_vertex: graph.get_neighbors(current))
        {
...
bool dfs_cycle_detection(graph<V, E, graph_type::UNDIRECTED> const& graph)
{
    ...
    for(auto const& vertex: graph.get_vertices())
    {
        if(!visited_vertices.contains(vertex.first) &&
           detail::do_dfs_undirected(graph,
                                     visited_vertices,
                                     parent_vertices,
                                     vertex.first,
