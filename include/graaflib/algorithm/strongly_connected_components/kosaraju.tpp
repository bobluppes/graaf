...
namespace
{
    ...
    void do_visit_vertex(vertex_id_t const vertex_id,
                         directed_graph<V, E> const& graph,
                         ...
                         std::unordered_set<vertex_id_t>& seen_vertices)
    {
        if(!seen_vertices.contains(vertex_id))
        {
            ...
            for(auto const neighbour: graph.get_neighbors(vertex_id))
            {
    ...
    void make_strongly_connected_component_from_vertex(
        vertex_id_t const vertex,
        directed_graph<V, E> const& transposed_graph,
        ...
        std::unordered_set<vertex_id_t>& seen_vertices)
    {
        if(!seen_vertices.contains(vertex))
        {
            ...
            for(auto const neighbour: transposed_graph.get_neighbors(vertex))
            {
                make_strongly_connected_component_from_vertex(
                    neighbour, transposed_graph, scc, seen_vertices);
            }
        }
    }
...
sccs_t kosarajus_strongly_connected_components(directed_graph<V, E> const& graph)
{
    ...
    auto const transposed_graph = get_transposed_graph(graph);
    std::vector<vertex_id_t> scc {};
