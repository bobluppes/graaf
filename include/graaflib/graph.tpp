...
std::size_t graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::vertex_count() const noexcept
{
...
template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
VERTEX_T& graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::get_vertex(vertex_id_t vertex_id)
{
    // Effective C++ item 3: we can safely call a const function from a non-const.
    // one
    return const_cast<VERTEX_T&>(
        const_cast<graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V> const*>(this)->get_vertex(
            vertex_id));
}
...
template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
typename graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::edge_t& graph<
    VERTEX_T,
    EDGE_T,
    GRAPH_TYPE_V>::get_edge(vertex_id_t vertex_id_lhs, vertex_id_t vertex_id_rhs)
{
    // Effective C++ item 3: we can safely call a const function from a non-const.
    return const_cast<graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::edge_t&>(
        const_cast<graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V> const*>(this)->get_edge(
            vertex_id_lhs, vertex_id_rhs));
}

template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
typename graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::edge_t const& graph<
    VERTEX_T,
    EDGE_T,
    GRAPH_TYPE_V>::get_edge(vertex_id_t vertex_id_lhs, vertex_id_t vertex_id_rhs) const
{
...
typename graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::edge_t& graph<
    VERTEX_T,
    EDGE_T,
    GRAPH_TYPE_V>::get_edge(edge_id_t const& edge_id)
{
    auto const [vertex_id_lhs, vertex_id_rhs] {edge_id};
...
typename graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::edge_t const& graph<
    VERTEX_T,
    EDGE_T,
    GRAPH_TYPE_V>::get_edge(edge_id_t const& edge_id) const
{
    auto const [vertex_id_lhs, vertex_id_rhs] {edge_id};
    return get_edge(vertex_id_lhs, vertex_id_rhs);
}
...
template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
typename graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::vertices_t graph<
    VERTEX_T,
    EDGE_T,
    GRAPH_TYPE_V>::get_neighbors(vertex_id_t vertex_id) const
{
...
vertex_id_t graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::add_vertex(auto&& vertex)
{
    ...
    auto const vertex_id {vertex_id_supplier_};
...
template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
void graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::remove_vertex(vertex_id_t vertex_id)
{
    if(adjacency_list_.contains(vertex_id))
    {
...
template<typename VERTEX_T, typename EDGE_T, graph_type GRAPH_TYPE_V>
void graph<VERTEX_T, EDGE_T, GRAPH_TYPE_V>::remove_edge(vertex_id_t vertex_id_lhs,
                                                        vertex_id_t vertex_id_rhs)
{
    using enum graph_type;
    if constexpr(GRAPH_TYPE_V == DIRECTED)
