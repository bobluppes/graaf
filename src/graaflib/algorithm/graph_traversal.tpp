...
namespace detail {
  ...
  void do_dfs(
    ...
    for (auto neighbor_vertex : graph.get_neighbors(current)) {
      if (!seen_vertices.contains(neighbor_vertex)) {
        do_dfs(graph, seen_vertices, neighbor_vertex,
                                  callback);
      }
    }
  }
  ...
  template <typename V, typename E, GRAPH_SPEC S, typename CALLBACK_T>
  void do_bfs(const graph<V, E, S>& graph, std::unordered_set<vertex_id_t>& seen_vertices, vertex_id_t start_vertex, const CALLBACK_T& callback) {
    ...
    const auto enqueue_neighbors{[&graph, &to_explore](const vertex_id_t vertex) {
      std::ranges::for_each(graph.get_neighbors(vertex), [&to_explore](const vertex_id_t neighbor) {
